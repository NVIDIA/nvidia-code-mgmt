...
void APResource::startWatchingApEid() noexcept
{
    ...
    try
    {
        apMCTPService = dbusUtil.getServices(objPath.c_str(), mctpEndpointIntfName)[0];
    }
    ...
    if (apMCTPService.empty())
    {
        ...
        mctpApObjManagerMatch.emplace_back(bus, MatchRules::interfacesAdded("/xyz/openbmc_project/mctp"),
    ...
    deviceMatches.emplace_back(bus, MatchRules::propertiesChanged(objPath.c_str(),
        mctpEndpointEnableIntfName),
    std::bind(&APResource::onMCTPDiscoveryMsg, this,
              std::placeholders::_1));
    deviceMatches.emplace_back(bus, MatchRules::interfacesAdded(objPath.c_str()),
    std::bind(&APResource::onMCTPDiscoveryMsg, this,
              std::placeholders::_1));
...
{
    return erotResource->isDeviceEnumerated() and erotResource->checkForEnabledMCTPEids();
}
