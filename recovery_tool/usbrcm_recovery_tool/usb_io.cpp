...
namespace {
   ...
   */
  constexpr uint32_t littleEndianToUint32(const std::array<uint8_t, 4>& data) noexcept {
  ...
  constexpr uint8_t REQUEST_TYPE_WRITE = 0x40;     ///< Host-to-device, vendor-specific
...
std::optional<uint32_t> readRegister32(libusb_device_handle* handle,
  ...
  if (verbose) {
    ...
    if (bytesRead < 0) {
      ...
      // Partial read
      std::cerr << "Incomplete read (expected "
                << REGISTER_SIZE_BYTES << " bytes, got "
                << bytesRead << " bytes)\n";
    }
  }
...
readDeviceEcid(libusb_device_handle* handle, bool verbose) noexcept {
  ...
  if (descResult < 0) {
    if (verbose) {
      std::cerr << std::format("readDeviceEcid: Failed to get device descriptor: {}\n",
  ...
  if (desc.iSerialNumber == 0) {
    if (verbose) {
      std::cerr << "readDeviceEcid: Device has no serial number descriptor\n";
    }
    return std::nullopt;
  }
  ...
  if (serialResult < 0) {
    if (verbose) {
      std::cerr << std::format("readDeviceEcid: Failed to read serial descriptor: {}\n",
  ...
  if (static_cast<size_t>(serialResult) < ECID_HEX_LENGTH) {
    if (verbose) {
      std::cerr << std::format("readDeviceEcid: Invalid ECID serial string length: {} (expected {})\n",
  ...
  const std::string_view hexView(reinterpret_cast<const char*>(serialBuffer.data()), 
                                 ECID_HEX_LENGTH);
  ...
  for (size_t i = 0; i < EcidParser::ECID_SIZE; ++i) {
    ...
    uint8_t byteValue = 0;
    const auto [ptr, ec] = std::from_chars(byteStart, byteEnd, byteValue, 16);
    
    if (ec != std::errc() || ptr != byteEnd) {
      if (verbose) {
        std::cerr << std::format("readDeviceEcid: Invalid hex character in ECID at position {}\n",
                                 hexOffset);
      }
      return std::nullopt;
    }
