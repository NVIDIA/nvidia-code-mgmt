...
public:
  ...
  [[nodiscard]] bool isValid() const noexcept { return context != nullptr; }
  [[nodiscard]] explicit operator bool() const noexcept { return isValid(); }
  [[nodiscard]] libusb_context* get() const noexcept { return context; }
 ...
 * 
 * Manages libusb_device_handle lifetime with automatic cleanup. Opens device, claims
 * interface on construction. Releases interface and closes device on destruction.
 * Also caches the USB port path at construction time for efficient access.
 */
/// USB interface constants
constexpr int INTERFACE_CONTROL = 0;   ///< Control interface for status/ECID reads
constexpr int INTERFACE_RECOVERY = 3;  ///< Recovery interface for RCM image transfers
...
public:
   ...
   * @param interfaceNum Interface number to claim (use INTERFACE_CONTROL or INTERFACE_RECOVERY)
   */
  UsbDeviceHandle(libusb_device* device, const UsbContext& ctx, int interfaceNum) noexcept;
  ...
  UsbDeviceHandle(UsbDeviceHandle&& other) noexcept
      : handle(other.handle), portPath(std::move(other.portPath)), claimedInterface(other.claimedInterface) {
  ...
  [[nodiscard]] explicit operator bool() const noexcept { return isValid(); }
  [[nodiscard]] libusb_device_handle* get() const noexcept { return handle; }
   ...
   */
  [[nodiscard]] const std::string& getPortPath() const noexcept { return portPath; }
 ...
 * 
 * Automatically manages libusb_device reference counting. Device is unreferenced
 * in destructor. Move-only type to prevent double-unref.
 */
class UsbDevice {
public:
  ...
  // Accessors
  [[nodiscard]] libusb_device* get() const noexcept { return device; }
  [[nodiscard]] const std::string& getPortPath() const noexcept { return portPath; }
 ...
 * 
 * @note Memory Management: Fully automatic via RAII. Device references are managed
 *       by UsbDevice objects and automatically cleaned up when they go out of scope.
 * 
 ...
 */
[[nodiscard]] std::vector<UsbDevice> findDevicesByVidPid(
    const UsbContext& ctx, 
 ...
 */
[[nodiscard]] std::optional<UsbDevice> findDeviceByPortPath(
    const UsbContext& ctx,
    std::string_view portPath,
    bool verbose = false) noexcept;

/**
 * @brief Validate device is in RCM mode (VID:PID + interface 3 + endpoint 0x08)
 * 
 * Performs comprehensive validation: VID:PID match AND recovery interface/endpoint presence.
 * 
 * @param device USB device pointer to check (must be valid)
 * @param verbose Enable diagnostic output (default: false)
 * @return true if device has correct VID:PID AND recovery endpoints, false otherwise
 */
[[nodiscard]] bool isDeviceInRcmMode(libusb_device* device, bool verbose = false) noexcept;
