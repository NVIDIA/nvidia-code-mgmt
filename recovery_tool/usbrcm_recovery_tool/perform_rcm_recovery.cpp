...
constexpr uint8_t RCM_BULK_OUT_ENDPOINT = 0x08;  ///< Bulk OUT endpoint for images
...
constexpr int POST_IMAGE_MONITOR_ITERATIONS = 4;          ///< Monitor for 2 seconds after image (4 × 500ms)
...
[[nodiscard]] bool sendBulkData(libusb_device_handle *handle,
  ...
  while (totalSent < size) {
    ...
    const int ret = libusb_bulk_transfer(
        handle,
        RCM_BULK_OUT_ENDPOINT,
        const_cast<uint8_t*>(data + totalSent),
    ...
    if (ret != LIBUSB_SUCCESS) {
      if (verbose) {
        std::cerr << std::format("Bulk transfer failed: {} ({})\n",
    ...
    if (actualLength <= 0) {
      if (verbose) {
        std::cerr << "Bulk transfer succeeded but transferred 0 bytes\n";
      }
      return false;
    }
 ...
 * @param lastProcessedTimestamp Reference to timestamp of last processed code; updated as new codes are processed
 * @param errorDetail Reference to error message string; populated if error code detected
 ...
 */
[[nodiscard]] ProgressStatus checkForNewProgressCodes(libusb_device_handle *handle,
                              uint32_t &lastProcessedTimestamp,
                              std::string &errorDetail,
                              bool verbose) {
  std::vector<progress_queue::CPUProgressLogEntry> entries;
  
  if (!progress_queue::readCpuPcqEntriesAfter(handle, lastProcessedTimestamp, entries, false)) {
  ...
  for (const auto& entry : entries) {
    if (entry.progressCode == 0) continue;
    
    const auto info = ProgressCodeParser::getProgressCodeInfo(entry.progressCode);
    
    if (verbose) {
      if (info.type == ProgressCodeParser::CodeType::Error) {
        std::cerr << std::format("ERROR [CPU{}]: {}\n", 
 ...
 * 
 * Polls for new progress codes every 500ms for a total of 2 seconds (4 iterations).
 * Returns immediately if completion code or error is detected during monitoring.
 * 
 * @param handle USB device handle for reading progress codes
 * @param lastProcessedTimestamp Reference to timestamp of last processed code; updated as new codes are processed
 * @param errorDetail Reference to error message string; populated if error code detected
 ...
 * @return ProgressStatus::Error if error code detected
 * @return ProgressStatus::Continue if monitoring period elapses without completion/error
 */
[[nodiscard]] ProgressStatus monitorProgressAfterImageTransfer(libusb_device_handle *handle,
                                uint32_t &lastProcessedTimestamp,
                                std::string &errorDetail,
                                bool verbose) {
  ...
  for (int i = 0; i < POST_IMAGE_MONITOR_ITERATIONS; ++i) {
    ...
    ProgressStatus status = checkForNewProgressCodes(handle, lastProcessedTimestamp, errorDetail, verbose);
...
bool performUsbRecovery(const std::string &portPath,
  ...
  try {
    ...
    if (!device) {
      jsonOutput["Status"] = "Failed";
      jsonOutput["Error"] = std::format("Device not found at port path: {}", portPath);
      return false;
    }
    
    usb::UsbDeviceHandle deviceHandle(device->get(), ctx, usb::INTERFACE_RECOVERY);
    if (!deviceHandle) {
      jsonOutput["Status"] = "Failed";
      jsonOutput["Error"] = std::format("Failed to open USB device and claim RCM interface {}", usb::INTERFACE_RECOVERY);
      return false;
    }
    ...
    if (verbose) {
      std::cout << std::format("Opened device at port {} and claimed RCM interface {}\n", 
    ...
    if (auto ecid = usb_io::readDeviceEcid(rawHandle, verbose)) {
      ...
      if (verbose) {
        std::cerr << "Warning: Could not read ECID, proceeding without blob validation\n";
      }
    }
    
    finalImagePaths.insert(finalImagePaths.end(), imagePaths.begin(), imagePaths.end());
    ...
    for (size_t i = 0; i < finalImagePaths.size(); ++i) {
      ...
      if (verbose) {
        std::cout << std::format("Sending image {} of {}: {}\n",
                                 i + 1, finalImagePaths.size(), imagePath);
      }
      
      std::vector<uint8_t> imageData;
      if (!readFileToBuffer(imagePath, imageData, verbose)) {
        jsonOutput["Status"] = "Failed";
        jsonOutput["Error"] = std::format("Failed to read image file: {}", imagePath);
        return false;
      }
      ...
      if (!sendBulkData(rawHandle, imageData.data(), imageData.size(), verbose)) {
        jsonOutput["Status"] = "Failed";
        jsonOutput["Error"] = std::format("Failed to send image: {}", imagePath);
        return false;
      }
      
      if (verbose) {
        std::cout << std::format("  Image {} sent successfully\n", i + 1);
        std::cout << std::format("Monitoring progress after image {}...\n", i + 1);
      }
      ProgressStatus status = monitorProgressAfterImageTransfer(rawHandle, lastProcessedTimestamp, errorDetail, verbose);
      ...
      if (i < finalImagePaths.size() - 1) {
        std::this_thread::sleep_for(std::chrono::milliseconds(INTER_IMAGE_DELAY_MS));
      }
    }
