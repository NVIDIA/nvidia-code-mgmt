...
UsbContext::UsbContext() noexcept {
  ...
  if (result != LIBUSB_SUCCESS) {
    std::cerr << std::format("Failed to initialize libusb: {} ({})\n",
...
namespace {
   ...
   */
  [[nodiscard]] bool hasCorrectVidPid(libusb_device* device, const bool verbose) noexcept {
    if (!device) {
      return false;
    }
    ...
    const bool matches = (desc.idVendor == VENDOR_ID && desc.idProduct == PRODUCT_ID);
    
    if (verbose && !matches) {
      std::cout << std::format("Device VID:PID = 0x{:04X}:0x{:04X} (expected 0x{:04X}:0x{:04X})\n",
   ...
   */
  [[nodiscard]] bool hasRecoveryEndpoints(libusb_device* device, const bool verbose) noexcept {
    if (!device) {
      return false;
    }
    ...
    std::unique_ptr<libusb_config_descriptor, decltype(configGuard)> guard(config, configGuard);

    // Search for recovery interface and endpoint using modern C++
    const auto interfaces = std::span(config->interface, config->bNumInterfaces);
    
    for (const auto& interface : interfaces) {
      const auto altsettings = std::span(interface.altsetting, interface.num_altsetting);
      
      for (const auto& altsetting : altsettings) {
        if (altsetting.bInterfaceNumber == INTERFACE_RECOVERY) {
          const auto endpoints = std::span(altsetting.endpoint, altsetting.bNumEndpoints);
          
          const bool found = std::any_of(endpoints.begin(), endpoints.end(),
                                         [](const auto& ep) {
                                           return (ep.bEndpointAddress & ENDPOINT_ADDRESS_MASK) == ENDPOINT_RECOVERY;
                                         });
    ...
    // Not found
    if (verbose) {
      std::cout << std::format("Device does not have recovery interface {} with endpoint 0x{:02X}\n",
  ...
  [[nodiscard]] std::string getDevicePortPath(libusb_device* device) noexcept {
    ...
    try {
      ...
      const int portCount = libusb_get_port_numbers(device, ports.data(), 
      ...
      const auto validPorts = std::span(ports.data(), static_cast<size_t>(portCount));
...
UsbDeviceHandle::UsbDeviceHandle(libusb_device* device, const UsbContext& ctx, int interfaceNum) noexcept
  ...
  if (ret != LIBUSB_SUCCESS) {
    if (portPath.empty()) {
      std::cerr << std::format("Failed to open USB device: {} ({})\n",
                                libusb_error_name(ret),
                                libusb_strerror(static_cast<libusb_error>(ret)));
    } else {
      std::cerr << std::format("Failed to open USB device at port {}: {} ({})\n",
                                portPath,
                                libusb_error_name(ret),
                                libusb_strerror(static_cast<libusb_error>(ret)));
    }
  ...
  if (libusb_kernel_driver_active(handle, claimedInterface) == 1) {
    ...
    if (ret != LIBUSB_SUCCESS) {
      std::cerr << std::format("Warning: Failed to detach kernel driver from interface {}: {}\n",
                                claimedInterface,
                                libusb_error_name(ret));
    }
  }
  ...
  if (ret != LIBUSB_SUCCESS) {
    std::cerr << std::format("Failed to claim USB interface {}: {} ({})\n",
                              claimedInterface,
                              libusb_error_name(ret),
                              libusb_strerror(static_cast<libusb_error>(ret)));
    cleanup();
  ...
  if (handle) {
    if (claimedInterface >= 0) {
      ...
      if (ret != LIBUSB_SUCCESS && ret != LIBUSB_ERROR_NOT_FOUND) {
        // Log non-trivial errors (NOT_FOUND is normal if already released)
        std::cerr << "Warning: Failed to release interface " << claimedInterface 
                  << ": " << libusb_strerror(static_cast<libusb_error>(ret)) << '\n';
      }
      ...
      if (ret != LIBUSB_SUCCESS && ret != LIBUSB_ERROR_NOT_FOUND && 
        ...
        // Log non-trivial errors (NOT_FOUND/NOT_SUPPORTED are normal on some platforms)
        std::cerr << "Warning: Failed to reattach kernel driver for interface " 
                  << claimedInterface << ": " 
                  << libusb_strerror(static_cast<libusb_error>(ret)) << '\n';
      }
    }
...
bool isDeviceInRcmMode(libusb_device* device, const bool verbose) noexcept {
  ...
  return hasCorrectVidPid(device, verbose) && hasRecoveryEndpoints(device, verbose);
}

std::vector<UsbDevice> findDevicesByVidPid(const UsbContext& ctx, const bool verbose) noexcept {
  if (!ctx.isValid()) {
    if (verbose) {
  ...
  if (deviceCount < 0) {
    if (verbose) {
      std::cerr << std::format("Failed to get USB device list: {}\n",
                                libusb_error_name(static_cast<int>(deviceCount)));
    }
    return {};  // libusb_get_devicelist does NOT allocate list on error, safe to return
  }
  ...
  std::unique_ptr<libusb_device*, decltype(deviceListGuard)> guard(deviceList, deviceListGuard);

  if (verbose) {
    std::cout << std::format("Scanning {} USB devices for VID:PID=0x{:04X}:0x{:04X}...\n",
  ...
  for (ssize_t i = 0; i < deviceCount; i++) {
    if (hasCorrectVidPid(deviceList[i], verbose)) {
      libusb_ref_device(deviceList[i]);  // Increment ref count (caller owns reference)
      
      const std::string portPath = getDevicePortPath(deviceList[i]);
      matchingDevices.push_back({deviceList[i], portPath});
      
      if (verbose) {
        std::cout << std::format("Found device VID:PID=0x{:04X}:0x{:04X} at port {}\n",
  ...
  if (verbose) {
    std::cout << std::format("Found {} device(s) with VID:PID=0x{:04X}:0x{:04X}\n",
