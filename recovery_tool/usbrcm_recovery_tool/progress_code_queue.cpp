...
{
constexpr uint32_t QUEUE0_OFFSET =
    0x2000; ///< Queue 0 pointer register (SCRATCH_FF_L0_GRP2_0)
}
 ...
 *
 * Holds the metadata for navigating the circular Progress Code Queue in
 * hardware. These values are parsed from the queue pointer register and define
 * the valid range of entries to read.
 */
struct CpuPcqPointers
{
    uint32_t
        endIndex; ///< bits [9:0]   - Next location to be written by hardware
    uint32_t startIndex; ///< bits [19:10] - Location with oldest valid entry
    uint32_t
        queueSize; ///< bits [30:20] - Total queue capacity (1-1024 entries)
};
...
/**
 * @brief Check if timestamp is invalid (hardware uninitialized or overflow
 * marker)
 */
[[nodiscard]] constexpr bool isInvalidTimestamp(uint32_t timestamp) noexcept
{
    return timestamp == INVALID_TIMESTAMP_ZERO ||
           timestamp == INVALID_TIMESTAMP_MAX;
}
...
[[nodiscard]] constexpr uint32_t
    advanceCircularIndex(uint32_t currentIdx, uint32_t baseIdx,
 ...
 */
[[nodiscard]] constexpr CpuPcqPointers
    parseCpuPcqPointers(uint32_t regValue) noexcept
{
    return {.endIndex = regValue & END_INDEX_MASK,
 ...
 */
[[nodiscard]] std::optional<CpuPcqPointers>
    readCpuPcqPointers(libusb_device_handle* handle, const bool verbose)
{
    if (!handle)
    {
        if (verbose)
    ...
    auto regValue =
        readRegister32(handle, QueuePointer::QUEUE0_OFFSET, verbose);
                       ...
                       const bool verbose)
{
    ...
    // For this hardware: write pointer == read pointer means BOTH empty queue
    // AND full queue Disambiguate: if queueSize != 0 and start == end, queue is
    // FULL (not empty)
    const bool isQueueFull = (curStart == curEnd) && (pointers->queueSize != 0);

    // Calculate number of entries to read
    const uint32_t entriesToRead =
        isQueueFull
            ? pointers->queueSize
    ...
    for (uint32_t i = 0; i < entriesToRead && i < MAX_ITERATION_LIMIT; ++i)
    {
        ...
        constexpr uint32_t maxSafeIndex =
            (UINT32_MAX - MemoryWindow::WINDOW_3_BASE) / ENTRY_SIZE_BYTES;
        ...
        const uint32_t timestampAddr =
            MemoryWindow::WINDOW_3_BASE + (readIdx * ENTRY_SIZE_BYTES);
        ...
        if (!timestamp || !progressCode)
        {
            if (verbose)
            {
                std::cerr << "Failed to read entry at index " << readIdx
                          ...
                          << ", progress code addr: 0x" << progressCodeAddr
                          << std::dec << ")\n";
            }
            return false;
        }
        ...
        {
            readIdx =
                advanceCircularIndex(readIdx, qbaseIdx, pointers->queueSize);
            continue;
        }
    ...
    if (entriesToRead > MAX_ITERATION_LIMIT)
    {
        ...
        {
            std::cerr
                << "Warning: Truncated read due to iteration limit. Wanted "
    ...
    std::sort(entries.begin(), entries.end(),
              [](const CPUProgressLogEntry& a,
                 const CPUProgressLogEntry& b) noexcept {
...
CPUBootSnapshot
    getCPUBootSnapshot(const std::vector<CPUProgressLogEntry>& entries)
{
    ...
    // Phase 1: Find the latest BOOT_MODE_SEL_DONE (search from newest to
    // oldest)
    auto bootModeIt = std::find_if(
        entries.rbegin(), entries.rend(), [](const CPUProgressLogEntry& entry) {
            const auto info = getProgressCodeInfo(entry.progressCode);
            return info.type == CodeType::Progress &&
                   info.subClass == SubClass::PSC_ROM &&
                   info.operation ==
                       static_cast<uint8_t>(
                           PscRomProgressCode::BOOT_MODE_SEL_DONE);
        });
    ...
    // Phase 2: From BOOT_MODE_SEL_DONE to newest entry, find latest progress
    // and error codes Note: BOOT_MODE_SEL_DONE itself may qualify as the latest
    // progress code

    // Search from newest entry backwards to BOOT_SEL (inclusive)
    // We need to find where to stop: one position past bootModeIt (towards
    // older entries)
    ...
    auto progressIt = std::find_if(
        entries.rbegin(), searchEnd, [](const CPUProgressLogEntry& entry) {
            return getProgressCodeInfo(entry.progressCode).type ==
                   CodeType::Progress;
        });
