 ...
 * @param virtualAddress Virtual address to read from (full 32-bit address space)
 ...
 * @endcode
 */
[[nodiscard]] std::optional<uint32_t> readRegister32(libusb_device_handle* handle,
...
[[nodiscard]] std::optional<std::array<uint8_t, EcidParser::ECID_SIZE>>
    readDeviceEcid(libusb_device_handle* handle,
                   bool verbose = false) noexcept;
