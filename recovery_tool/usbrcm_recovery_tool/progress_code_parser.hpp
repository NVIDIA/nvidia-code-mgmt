 ...
 *   info.bootSelectionInfo // Optional boot selection info (for BOOT_MODE_SEL_DONE)
 *   info.getBootSelectionString()  // Boot selection details (empty if not available)
 *
 * Example:
 ...
 *   }
 *   std::cout << "Code: " << info.name;  // "PSC_ROM_PC_BOOT_MODE_SEL_DONE" or "0xXXXXXXXX"
 */
...
struct ProgressCodeInfo {
    ...
    // Boot selection information (only present when progress code is PSC_ROM_PC_BOOT_MODE_SEL_DONE)
...
constexpr std::string_view codeTypeToString(CodeType type) {
    ...
    switch (type) {
        case CodeType::Progress: return "Progress"sv;
        case CodeType::Error: return "Error"sv;
        case CodeType::Debug: return "Debug"sv;
    }
    return "Unknown"sv;
}
...
constexpr std::string_view packageClassToString(PackageClass pkg) {
    using namespace std::string_view_literals;
    switch (pkg) {
        case PackageClass::Package0: return "CPU0"sv;
        case PackageClass::Package1: return "CPU1"sv;
        case PackageClass::Unknown: return "Unknown"sv;
    }
    return "Unknown"sv;
}
...
constexpr std::string_view subClassToString(SubClass subclass) {
    using namespace std::string_view_literals;
    switch (subclass) {
        case SubClass::PSC_ROM: return "PSC_ROM"sv;
        case SubClass::PSC_FMC: return "PSC_FMC"sv;
        case SubClass::PSC_RT: return "PSC_RT"sv;
        case SubClass::MB1: return "MB1"sv;
        case SubClass::BPMP_FW: return "BPMP_FW"sv;
        case SubClass::MB2: return "MB2"sv;
        case SubClass::ATF_BL31: return "ATF_BL31"sv;
        case SubClass::RMM: return "RMM"sv;
        case SubClass::Hafnium: return "Hafnium"sv;
        case SubClass::UEFI: return "UEFI"sv;
        case SubClass::UEFI_StMM: return "UEFI_StMM"sv;
        case SubClass::OOBHUB_FW: return "OOBHUB_FW"sv;
        case SubClass::RAS_FW: return "RAS_FW"sv;
        case SubClass::MSEQ_FW: return "MSEQ_FW"sv;
        case SubClass::PCORE0_FW: return "PCORE0_FW"sv;
        case SubClass::PCORE1_FW: return "PCORE1_FW"sv;
        case SubClass::PCORE2_FW: return "PCORE2_FW"sv;
        case SubClass::PCORE3_FW: return "PCORE3_FW"sv;
        case SubClass::PCORE4_FW: return "PCORE4_FW"sv;
        case SubClass::PCORE5_FW: return "PCORE5_FW"sv;
        case SubClass::C2C_GRS_0: return "C2C_GRS_0"sv;
        case SubClass::C2C_GRS_1: return "C2C_GRS_1"sv;
        case SubClass::C2C_UPHY_0: return "C2C_UPHY_0"sv;
        case SubClass::C2C_UPHY_1: return "C2C_UPHY_1"sv;
        case SubClass::C2C_UPHY_2: return "C2C_UPHY_2"sv;
        case SubClass::C2C_UPHY_3: return "C2C_UPHY_3"sv;
        case SubClass::C2C_UPHY_4: return "C2C_UPHY_4"sv;
        case SubClass::C2C_UPHY_5: return "C2C_UPHY_5"sv;
        case SubClass::C2C_LPI_S_0: return "C2C_LPI_S_0"sv;
        case SubClass::C2C_LPI_S_1: return "C2C_LPI_S_1"sv;
        case SubClass::C2C_LPI_C_0: return "C2C_LPI_C_0"sv;
        case SubClass::C2C_LPI_C_1: return "C2C_LPI_C_1"sv;
    }
    return "Unknown"sv;
}
...
constexpr std::string_view bootModeToString(BootMode mode) {
    using namespace std::string_view_literals;
    switch (mode) {
        case BootMode::Recovery: return "Recovery"sv;
        case BootMode::ColdbootOrStreaming: return "Coldboot/Streaming"sv;
    }
    return "Unknown"sv;
}
...
constexpr std::string_view bootDeviceToString(BootDevice device) {
    using namespace std::string_view_literals;
    switch (device) {
        case BootDevice::Invalid: return "Invalid"sv;
        case BootDevice::QSPI: return "QSPI"sv;
        case BootDevice::OCP_RC: return "OCP_RC"sv;
        case BootDevice::USB: return "USB"sv;
    }
    return "Unknown"sv;
}
...
constexpr std::string_view selectionTypeToString(SelectionType type) {
    using namespace std::string_view_literals;
    switch (type) {
        case SelectionType::STRAP: return "STRAP"sv;
        case SelectionType::OCP_CMD: return "OCP_CMD"sv;
        case SelectionType::FUSE: return "FUSE"sv;
        case SelectionType::BCR: return "BCR"sv;
    }
    return "Unknown"sv;
}
...
constexpr std::string_view recoveryTypeToString(RecoveryType type) {
    using namespace std::string_view_literals;
    switch (type) {
        case RecoveryType::ErrorInitiated: return "Error-Initiated"sv;
        case RecoveryType::Forced: return "Forced"sv;
    }
    return "Unknown"sv;
}
 ...
 * @note Only meaningful for PSC_ROM_PC_BOOT_MODE_SEL_DONE (SubClass=PSC_ROM, Operation=0x02)
 */
...
/**
 * @brief Decode BOOT_SEL_VAL into structured information with cached string representation
 * @param bootSelVal 7-bit BOOT_SEL_VAL value
 * @return BootSelInfo structure with all fields including cached string
 * @note bootSelVal is meaningful when the progress code is from PSC_ROM_PC_BOOT_MODE_SEL_DONE
 */
inline BootSelInfo decodeBootSelVal(uint16_t bootSelVal) {
    ...
    const auto selectionType = static_cast<SelectionType>((bootSelVal >> 4) & 0x03);
    const auto recoveryType = static_cast<RecoveryType>((bootSelVal >> 6) & 0x01);
    ...
    return BootSelInfo{
        ...
        .stringRepresentation = std::format(
            ...
            bootModeToString(bootMode),
            bootDeviceToString(bootDevice),
            deviceState == DeviceState::Enabled ? "Enabled" : "Disabled",
            selectionTypeToString(selectionType),
            bootMode == BootMode::Recovery
                ? recoveryTypeToString(recoveryType)
                : "")
    };
}
...
constexpr uint8_t packageClassToCpuId(PackageClass pkg) {
    switch (pkg) {
        case PackageClass::Package0: return 0;
        case PackageClass::Package1: return 1;
        case PackageClass::Unknown: return 255;
    }
    return 255;
}
 ...
 */
inline std::string mapOperationToString(CodeType codeType, SubClass subClass, uint8_t operation) {
    if (subClass == SubClass::PSC_ROM) {
        if (codeType == CodeType::Error) {
            if (operation < Detail::PSC_ROM_ERROR_COUNT && !Detail::psc_rom_error_reasons[operation].empty()) {
                return std::string(Detail::psc_rom_error_reasons[operation]);
            }
        } else if (codeType == CodeType::Progress) {
            if (operation < Detail::PSC_ROM_PROGRESS_COUNT && !Detail::psc_rom_progress_names[operation].empty()) {
                return std::string(Detail::psc_rom_progress_names[operation]);
            }
        }
    } else if (subClass == SubClass::PSC_FMC) {
        if (codeType == CodeType::Error) {
            if (operation < Detail::PSC_FMC_ERROR_COUNT && !Detail::psc_fmc_error_reasons[operation].empty()) {
                return std::string(Detail::psc_fmc_error_reasons[operation]);
            }
        } else if (codeType == CodeType::Progress) {
            if (operation < Detail::PSC_FMC_PROGRESS_COUNT && !Detail::psc_fmc_progress_names[operation].empty()) {
