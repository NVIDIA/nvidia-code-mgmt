...
namespace nvidia::smcu::device
{

using SmcuInherit = sdbusplus::server::object::object<
 ...
 * @class Smcu
 */
class Smcu : public SmcuInherit, public Util
{
  public:
    Smcu() = delete;
    Smcu(const Smcu&) = delete;
    Smcu(Smcu&&) = delete;
    Smcu& operator=(const Smcu&) = delete;
    Smcu& operator=(Smcu&&) = delete;
    ~Smcu() = default;

    Smcu(sdbusplus::bus::bus& bus, const std::string& objPath) :
        SmcuInherit(bus, (objPath).c_str()),
    bus(bus),
                                 ...
                                 const std::string& ifPath)
    {
        ...
        swpath += "/" + fName;
        std::string smcuVersion = getVersion();
        VersionObj =
            std::make_unique<VersionInterface>(bus, swpath, smcuVersion);
    }
